...
function App() {
  ...
  // Animate background orbs on theme change
  useEffect(() => {
    if (!orbsRef.current || !isReady) return

    const orbs = orbsRef.current.querySelectorAll('.bg-orb')
    
    orbs.forEach((orb, index) => {
      const colors = theme === 'dark' 
        ? ['rgba(72, 239, 207, 0.08)', 'rgba(240, 78, 152, 0.08)', 'rgba(11, 100, 221, 0.05)']
        : ['rgba(11, 100, 221, 0.12)', 'rgba(240, 78, 152, 0.1)', 'rgba(72, 239, 207, 0.08)']
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden transition-colors duration-300 ${
      ...
      <div ref={orbsRef} className="fixed inset-0 overflow-hidden pointer-events-none">
        <div 
          className="bg-orb absolute top-1/4 -left-1/4 w-[600px] h-[600px] rounded-full blur-[120px]"
          style={{
            backgroundColor: theme === 'dark' ? 'rgba(72, 239, 207, 0.08)' : 'rgba(11, 100, 221, 0.12)'
          }}
        />
        <div 
          className="bg-orb absolute bottom-1/4 -right-1/4 w-[500px] h-[500px] rounded-full blur-[100px]"
          style={{
            backgroundColor: theme === 'dark' ? 'rgba(240, 78, 152, 0.08)' : 'rgba(240, 78, 152, 0.1)'
          }}
        />
        <div 
          className="bg-orb absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full blur-[150px]"
          style={{
